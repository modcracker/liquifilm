...
import { ChevronRight, Home, Key, BookOpen, ChevronLeft, ArrowRight, CornerDownRight, ScrollText } from 'lucide-react';
import SchemaMarkup from '@/components/SchemaMarkup';
import { WIDER_TOPICS } from '@/lib/data';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const page = WIDER_TOPICS.find((p) => p.slug === slug);
  if (!page) return {};
...
export async function generateStaticParams() {
  return WIDER_TOPICS.map((page) => ({
    slug: page.slug,
  }));
}

export default async function WiderTopicPage({ params }: Props) {
  const { slug } = await params;
  const pageIdx = WIDER_TOPICS.findIndex((p) => p.slug === slug);
  
  if (pageIdx === -1) {
    notFound();
  }

  const page = WIDER_TOPICS[pageIdx];
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": "Preservation Topics",
        "item": `${siteUrl}/topics/history-of-film-preservation`
      },
      {
        "@type": "ListItem",
        "position": 3,
  ...
  const articleSchema = {
    ...
    "image": "https://picsum.photos/seed/liquifilm_topic/1200/630",
    "articleSection": page.category || "Film Preservation Topics",
    "publisher": {
      "@type": "Organization",
      "name": "Liquifilm",
      "logo": {
      ...
      <div className="bg-white border-b border-neutral-100 py-4">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <nav className="flex items-center gap-2 text-xs font-mono text-neutral-500">
            ...
            <span className="text-neutral-500">Wider Topics</span>
            <ChevronRight className="h-3 w-3" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <aside className="lg:col-span-3 space-y-6">
            <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-3xs">
              <h3 className="font-display text-xs font-bold uppercase tracking-widest text-neutral-900 border-b border-neutral-200 pb-2 mb-4 flex items-center gap-2">
                <ScrollText className="h-4 w-4 text-neutral-500" /> Preservation Library
              </h3>
              <div className="space-y-1">
                {WIDER_TOPICS.map((topic) => {
                  ...
                  return (
                    ...
                    >
                      {topic.title.replace("The Global History of ", "").replace("The Economics of ", "")}
                    </Link>
                  );
                })}
              </div>
          ...
          <article className="lg:col-span-6 bg-white border border-neutral-200 rounded-2xl p-6 md:p-8 shadow-3xs">
            ...
            <div className="mb-6 select-none">
              <PlaceholderImage
                aspectRatio="16:9"
                category={page.slug.includes("history") ? "historical" : "laboratory"}
                altText={`Technical chart detailing the preservation topic of ${page.title}: ${page.summary}`}
              />
            </div>
            ...
            <div className="mt-12 pt-6 border-t border-neutral-200 flex flex-col sm:flex-row justify-between gap-4">
              {prevPage ? (
                ...
                >
                  ...
                  <div>
                    <span className="block text-[9px] font-mono text-neutral-400 uppercase font-semibold">Previous Topic</span>
              ...
              {nextPage ? (
                ...
                >
                  <div className="ml-auto">
                    <span className="block text-[9px] font-mono text-neutral-400 uppercase font-semibold">Next Topic</span>
          ...
          <aside className="lg:col-span-3 space-y-6">
            ...
            <div className="rounded-xl bg-neutral-100 border border-neutral-200 p-5">
              <h4 className="font-display text-xs font-bold text-neutral-800 mb-1">Standard Reference</h4>
              <p className="text-[11px] text-neutral-500 leading-relaxed">
                All data in this document is compiled from historic and contemporary film archives.
              </p>
            </div>
          </aside>
